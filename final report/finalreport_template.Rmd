...
```
```{r stacked, echo=FALSE, message=FALSE, warning=FALSE, out.width="65%", fig.align='center'}
knitr::include_graphics("finc_stacked.png")
```

Assessing both the boxplot and stacked barplots above for region and urbanness respectively, there does indeed appear to be a connection between NC Region and having a percent change of food insecurity rate greater than 10%, with nearly all Piedmont county regions, but less than 25% of Mountain and Coastal Plains counties, having a percent change of food insecurity rate greater than 10%. Meanwhile, based on the stacked barplot, it appears that Urban counties were more likely to have a percent change of food insecurity rate greater than 10%.
...
Significant results will be interpreted in context and and and ROC Curve will be fit to determine the predictiveness of this model on whether or not a county in NC has experienced a percent change of food insecurity rate greater than 10%.
...
summary(aov(`x100 2021 FI Percent`~Region,data=finc))
hairpair_fi21 <- pairwise.t.test(finc$`x100 2021 FI Percent`, finc$Region, p.adj = "holm")
sigpairs_fi21 <- broom::tidy(hairpair_fi21) %>%
  filter(p.value<0.05) %>%
  arrange(group1,group2) %>%
knitr::kable(caption= "Findings of Significant Findings of ANOVA of Food Insecurity Rates in North Carolina Counties, 2021")
nrow(sigpairs_fi21)
print(sigpairs_fi21, n=2)

summary(aov(FI_PC_Change~Region,data=finc))
hairpair_fipc <- pairwise.t.test(finc$FI_PC_Change, finc$Region, p.adj = "holm")
sigpairs_fipc <- broom::tidy(hairpair_fipc) %>%
  filter(p.value<0.05) %>%
  arrange(group1,group2) %>%
knitr::kable(caption= "Findings of Significant Findings of ANOVA of Food Insecurity Percent Change in North Carolina, 2019-2021")
nrow(sigpairs_fipc)
print(sigpairs_fipc, n=2)
```
